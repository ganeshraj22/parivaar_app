...
if page=='District Level':
    ...
    def get_data(selected_district,level_of_detail,sheet,location_global,date_range_dt):
        ...
        def agg_plots(ambulance_df1):
            if (location_global is None or location_global==[]):
                ...
                ambulance_df1['patients_location_sum']=ambulance_df1.loc[:,selected_locations_patients].sum()
                Patients_Pie=ambulance_df1[(ambulance_df1['Date']>=start_date)&(ambulance_df1['Date']<=end_date)].loc[:,location_global].sum()
                selected_locations_distance=selected_locations_patients.replace('/n','') +' (KM)'
                ambulance_df1['distance_location_sum']=ambulance_df1.loc[:,selected_locations_distance].sum()
            ...
            Ambulance_By_Month['Total Patients Served']=Ambulance_By_Month['patients_location_sum']
            Ambulance_By_Month['Total Distance Covered']=Ambulance_By_Month['distance_location_sum']
    ...
    with col2:
        ...
        locations.sort()
        locations.remove('All locations')
